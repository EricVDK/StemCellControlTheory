...
public class managerScriptFeedback: MonoBehaviour {
	...
	void Update () {
		...
		timerUpdate ();
		timerText.text = "Feedback Strategy: " + timer.ToString();
	}


	void createDroplet (){
		creationTimer += Time.deltaTime;
		if (creationTimer > creationRate) {
			creationTimer = 0;
		...
		for (int i = 0; i < B.Count; i++) {
			...
			if(droplet.GetComponent<Droplet>().currentTime > droplet.GetComponent<Droplet>().delay){
				...
				for (int j= gridCountC; j < gridC.Length; j++) {
					if (gridC [j]) {
					//	Debug.Log("woops, not there: " + j.ToString());
						gridCountC++;
						if (gridCountC > 99) {
							gridCountC = 0;
						}
	...
	void dropletExit(){
		...
		if (C.Count > 0) {
			if (exitTimer >= demandRate) {
				...
				gridC [droplet.GetComponent<Droplet> ().gridSpotC] = false;
				Debug.Log ("no worries, Spot: " + droplet.GetComponent<Droplet> ().gridSpotC.ToString() + " is now " + gridC [droplet.GetComponent<Droplet> ().gridSpotC].ToString());
				C.RemoveAt (rand);
				Destroy (droplet);
			}
		}
